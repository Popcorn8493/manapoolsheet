...
REPORT_PATTERNS = {
    'csv': "card_inventory_report_*.csv",
    'html': "manapoolshoot_*.html"
}
...
def sort_cards(cards: List[Card], sort_by: str, order: str,
               secondary_sort: Optional[str] = None,
               secondary_order: str = 'asc') -> List[Card]:
    from collections import Counter

    get_sort_key = create_sort_key_factory(sort_by)
    get_secondary_key = (create_sort_key_factory(secondary_sort)
                         if secondary_sort else None)
    ...
    if sort_by == 'location':
        ...
        if secondary_sort and get_secondary_key:
            cards.sort(
                key=lambda x: (get_sort_key(x), get_secondary_key(x)),
                reverse=reverse)
        else:
...
def download_and_cache_image(image_url: str, card_name: str,
                             set_code: str) -> Optional[str]:
    if not image_url:
        return None
...
def get_card_image_url(card_name: str, set_code: str,
                       collector_number: Optional[str] = None
                       ) -> Optional[str]:
    try:
        search_queries = build_scryfall_queries(
        ...
        for query in search_queries:
            try:
                ...
                session = get_session()
                response = session.get(SCRYFALL_BASE_URL, params=params,
                ...
                if data.get('data') and len(data['data']) > 0:
                    ...
                    image_url = (image_uris.get('large') or
...
def process_shipstation_data(input_file: str,
                             drawer_mapping: Dict[str, str]) -> List[Card]:
    cards = []
    ...
    with open(input_file, 'r', encoding='utf-8') as file:
        ...
        for row in reader:
            ...
            image_url = get_card_image_url(
                card_name, set_code, collector_number)

            cached_image_path = (download_and_cache_image(
                image_url, card_name, set_code
...
def generate_csv_report(cards: List[Card], output_file: str):
    with open(output_file, 'w', newline='', encoding='utf-8') as file:
        fieldnames = [
            'Card Name', 'Set', 'Set Code', 'Collector #', 'Quantity',
            'Condition', 'Rarity', 'Location', 'Unit Price',
            'Image URL'
        ]
...
def load_html_template(template_file: str = TEMPLATE_FILE
                       ) -> Optional[str]:
    try:
...
def render_card_html(card: Card) -> str:
    ...
    if card.image_url:
        image_html = (f'<div class="card-image-container">'
                      ...
                      f'alt="{card.name}" class="card-image">'
                      f'</div>')
    else:
...
def generate_html_report(cards: List[Card], output_file: str,
                         sort_by: str = 'location', order: str = 'asc'):
    ...
    for location in location_order:
        ...
        location_sections_html += render_location_section(
            location, cards_in_location)
    ...
    html_content = html_content.replace('{locations_count}',
                                        str(locations_count))
    html_content = html_content.replace('{location_sections}',
                                        location_sections_html)
...
def main():
    ...
    drawer_mapping = load_drawer_mapping(DRAWER_FILE)
    ...
    if args.secondary_sort:
        print(f"Secondary sort by {args.secondary_sort} "
              f"({args.secondary_order})...")
    cards = sort_cards(cards, args.sort_by, args.order, args.secondary_sort,
